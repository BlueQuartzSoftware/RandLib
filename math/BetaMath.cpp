...
long double logBeta(long double a, long double b)
{
  ...
  long double lgammaA = (a == roundA) ? RandMath::lfact(roundA - 1) : std::lgammal(a);
  long double lgammaB = lgammaA;
  if(a != b)
    lgammaB = (b == roundB) ? RandMath::lfact(roundB - 1) : std::lgammal(b);
  long double lgammaApB = (apb == roundApB) ? RandMath::lfact(roundApB - 1) : std::lgammal(apb);
...
double ibeta(double x, double a, double b, double logBetaFun, double logX, double log1mX)
{
  ...
  if(a <= 0 || b <= 0 || x < 0.0 || x > 1.0)
    throw std::invalid_argument("Parameters (a, b) of incomplete beta function should be both positive, but they're equal to " + std::to_string(a) + " and " + std::to_string(b) + " respectively");
  if(x < 0.0 || x > 1.0)
    throw std::invalid_argument("Argument x of incomplete beta function should be in interval [0, 1], but it's equal to " + std::to_string(x));
  if(x == 0.0)
    return 0.0;
  if(x == 1.0)
    return 1.0;
  ...
  if(x < 0.45)
  {
    ...
    double residue = (b == number) ? 0.0 : ibetaPowerSeries1(x, apn, bmn, RandMath::logBeta(apn, bmn), logX, log1mX);
    return ecd + residue;
  }
  ...
  double apn = a + number;
  double residue = ibetaPowerSeries2(x, apn, b, RandMath::logBeta(apn, b), logX, log1mX);
  return ecd + residue;
}
...
double ibeta(double x, double a, double b)
{
  ...
  double logBetaFun = RandMath::logBeta(a, b);
  double logX = std::log(x), log1mX = std::log1pl(-x);
  return RandMath::ibeta(x, a, b, logBetaFun, logX, log1mX);
}
