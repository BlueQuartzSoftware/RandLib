 ...
 */
constexpr int FACTORIAL_TABLESIZE = 175;

/**
 * @fn FACTORIAL_TABLE (n * 10)! for n from 0 to 17
 */
...
long double factorial(double n)
{
  if(n < 0)
    return 0.0;
  return (n > FACTORIAL_TABLESIZE) ? std::tgammal(n + 1) : factorialForSmallValue(n);
}

long double doubleFactorial(int n)
{
  long double nFact = RandMath::factorial(n);
  if(n & 1)
  {
    n <<= 1;
    return RandMath::factorial(n + 1) / (n * nFact);
  }
  return (1 << n) * nFact;
}

constexpr long double LOGFACTORIAL_TABLE[] = {0.l,
                                              0.l,
...
{
  if(n & 1)
    return RandMath::lfact(n) - RandMath::ldfact(n - 1);
  size_t k = n >> 1;
  return k * M_LN2 + RandMath::lfact(k);
}
...
long double binom(size_t n, size_t k)
{
  ...
  double lfactn = RandMath::lfact(n);
  double lfactk = RandMath::lfact(k);
  double lfactnmk = RandMath::lfact(n - k);
...
{
  /// set up tables
  static constexpr long double taylorCoef[] = {0.00833333333333333333l, -0.00396825396825396825l, 0.00416666666666666666l, -0.00757575757575757576l, 0.02115090296908478727l, -0.08333333333333333333l};
  static constexpr int bounds[] = {6000, 320, 75, 30, 20, 10, 7};
...
double digammamLog(double x)
{
  ...
  if(x == 0.0)
    return -INFINITY;
  return (x < 7.0) ? RandMath::digamma(x) - std::log(x) : digammamLogForLargeX(x);
}
...
double trigamma(double x)
{
  ...
  if(x < 0.0)
  {
    ...
    return z * z - RandMath::trigamma(1.0 - x);
  }
  /// set up tables
  ...
  static constexpr int bounds[] = {1000, 140, 47, 25, 15, 10};
...
double incompleteGammaUniformExpansion(double a, double x, double logX, double logA, bool isP)
{
  ...
  static constexpr long double d[] = {-0.33333333333333333333l,   0.08333333333333333333l,  -0.01481481481481481481l, 0.00115740740740740741l, 0.00035273368606701940l, -0.000178755144032922l,
                                      0.0000391926317852244l,     -0.00000218544851067999l, -0.00000185406221071516l, 0.829671134095309e-6l,   -0.176659527368261e-6l,  0.670785354340150e-8l,
                                      0.102618097842403e-7l,      -0.438203601845335e-8l,   0.914769958223678e-9l,    -0.255141939949460e-10l, -0.583077213255043e-10l, 0.243619480206674e-10l,
                                      -0.502766928011417e-11l,    0.110043920319559e-12l,   0.337176326240099e-12l,   -0.139238872241816e-12l, 0.285348938070474e-13l,  -0.513911183424242e-15l,
...
double lpgammaRaw(double a, double x, double logX, double logA, double lgammaA, REGULARISED_GAMMA_METHOD_ID mId)
{
  if(mId == PT)
  {
    ...
    for(int n = n0; n > 0; --n)
    {
      double addon = n * logX - std::lgammal(a + n + 1) + lgammaAp1;
      addon = std::exp(addon);
      sum += addon;
    }
    return a * logX - x + std::log1pl(sum) - lgammaAp1;
  }
  return (mId == PUA) ? std::log(pgammaRaw(a, x, logX, logA, lgammaA, mId)) : std::log1pl(-qgammaRaw(a, x, logX, logA, lgammaA, mId));
}

void parametersVerification(double a, double x)
{
  if(x < 0.0)
    throw std::invalid_argument("Argument x of gamma function should be non-negative, but it's equal to " + std::to_string(x));
  if(a < 0.0)
    throw std::invalid_argument("Parameter a of gamma function should be non-negative, but it's equal to " + std::to_string(a));
}

double lpgamma(double a, double x, double logA, double lgammaA)
{
  parametersVerification(a, x);
  if(x == 0.0)
    return 0.0;
  if(a == 1.0)
...
double pgammaRaw(double a, double x, double logX, double logA, double lgammaA, REGULARISED_GAMMA_METHOD_ID mId)
{
  ...
  return (mId == QUA) ? 1.0 - qgammaRaw(a, x, logX, logA, lgammaA, mId) : -std::expm1l(lqgammaRaw(a, x, logX, logA, lgammaA, mId));
}
...
double pgamma(double a, double x, double logX)
{
  ...
  if(a == 1.0)
    return -std::expm1l(-x);
  REGULARISED_GAMMA_METHOD_ID mId = getRegularizedGammaMethodId(a, x, logX);
  return pgammaRaw(a, x, logX, std::log(a), std::lgammal(a), mId);
}
...
double lqgammaRaw(double a, double x, double logX, double logA, double lgammaA, REGULARISED_GAMMA_METHOD_ID mId)
{
  ...
  return (mId == QUA) ? std::log(qgammaRaw(a, x, logX, logA, lgammaA, mId)) : std::log1pl(-pgammaRaw(a, x, logX, logA, lgammaA, mId));
}
...
double lqgamma(double a, double x, double logX)
{
  ...
  if(a == 1.0)
    return -x;
  REGULARISED_GAMMA_METHOD_ID mId = getRegularizedGammaMethodId(a, x, logX);
  return lqgammaRaw(a, x, logX, std::log(a), std::lgammal(a), mId);
}
...
double qgammaRaw(double a, double x, double logX, double logA, double lgammaA, REGULARISED_GAMMA_METHOD_ID mId)
{
  ...
  if(mId == QT)
    return -std::expm1l(qtGammaExpansionAux(a, logX, logA, lgammaA));
  if(mId == QUA)
    return incompleteGammaUniformExpansion(a, x, logX, logA, false);
  return (mId == PUA) ? 1.0 - pgammaRaw(a, x, logX, logA, lgammaA, mId) : -std::expm1l(lpgammaRaw(a, x, logX, logA, lgammaA, mId));
}
