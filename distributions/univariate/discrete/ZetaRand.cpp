...
template <typename IntType> ZetaRand<IntType>::ZetaRand(double exponent) {
  SetExponent(exponent);
}

template <typename IntType> String ZetaRand<IntType>::Name() const {
...
template <typename IntType> IntType ZetaRand<IntType>::Variate() const {
  /// Luc Devroye, p. 551
  /// rejection sampling from rounded down Pareto distribution
  size_t iter = 0;
  do {
    float X = std::floor(
        ParetoRand<float>::StandardVariate(sm1, this->localRandGenerator));
...
template <typename IntType> long double ZetaRand<IntType>::Mean() const {
  return (s > 2) ? std::riemann_zetal(sm1) / zetaS : INFINITY;
}

template <typename IntType> long double ZetaRand<IntType>::Variance() const {
  if (s <= 3)
    return INFINITY;
  double y = Mean();
...
template <typename IntType> IntType ZetaRand<IntType>::Mode() const {
  return 1;
}

template <typename IntType> long double ZetaRand<IntType>::Skewness() const {
  if (s <= 4)
    return INFINITY;
...
template <typename IntType> long double ZetaRand<IntType>::Moment(int n) const {
