...
YuleRand<IntType>::YuleRand(double shape) : X(shape, 1.0) {
  SetShape(shape);
}

template <typename IntType> String YuleRand<IntType>::Name() const {
  return "Yule(" + this->toStringWithPrecision(GetShape()) + ")";
}

template <typename IntType> void YuleRand<IntType>::SetShape(double shape) {
  if (shape <= 0.0)
...
template <typename IntType> IntType YuleRand<IntType>::Variate() const {
...
template <typename IntType> long double YuleRand<IntType>::Mean() const {
  return (rho <= 1) ? INFINITY : rho / (rho - 1);
}

template <typename IntType> long double YuleRand<IntType>::Variance() const {
  if (rho <= 2)
    return INFINITY;
...
template <typename IntType> IntType YuleRand<IntType>::Mode() const {
  return 1;
}

template <typename IntType> long double YuleRand<IntType>::Skewness() const {
  if (rho <= 3)
    return INFINITY;
