...
NegativeBinomialDistribution<IntType, T>::NegativeBinomialDistribution(
    T number, double probability) {
  SetParameters(number, probability);
}

template <typename IntType, typename T>
void NegativeBinomialDistribution<IntType, T>::SetParameters(
    T number, double probability) {
  if (r <= 0.0)
    throw std::invalid_argument(
        "Negative-Binomial distribution: number parameter should be positive");
...
template <typename IntType, typename T>
IntType
NegativeBinomialDistribution<IntType, T>::variateThroughGammaPoisson() const {
  return PoissonRand<IntType>::Variate(GammaRV.Variate(),
                                       this->localRandGenerator);
}

template <typename IntType, typename T>
IntType
NegativeBinomialDistribution<IntType, T>::variateGeometricByTable() const {
...
template <typename IntType, typename T>
IntType
NegativeBinomialDistribution<IntType, T>::variateGeometricThroughExponential()
    const {
  float X = -ExponentialRand<float>::StandardVariate(this->localRandGenerator) /
            log1mProb;
  return std::floor(X);
}
...
template <typename IntType, typename T>
IntType
NegativeBinomialDistribution<IntType, T>::variateThroughExponential() const {
  IntType var = 0;
  for (int i = 0; i < r; ++i) {
...
IntType NegativeBinomialDistribution<IntType, T>::Variate() const {
  ...
  return (genId == EXPONENTIAL) ? variateThroughExponential()
                                : variateThroughGammaPoisson();
}

template <typename IntType, typename T>
void NegativeBinomialDistribution<IntType, T>::Sample(
    std::vector<IntType> &outputData) const {
  GENERATOR_ID genId = GetIdOfUsedGenerator();
  if (genId == TABLE) {
...
template <typename IntType, typename T>
void NegativeBinomialDistribution<IntType, T>::Reseed(
...
template <typename IntType, typename T>
std::complex<double>
NegativeBinomialDistribution<IntType, T>::CFImpl(double t) const {
...
template <typename IntType, typename T>
BetaRand<> NegativeBinomialDistribution<IntType, T>::FitProbabilityBayes(
    const std::vector<IntType> &sample,
    const BetaDistribution<> &priorDistribution, bool MAP) {
  if (!this->allElementsAreNonNegative(sample))
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE, this->NON_NEGATIVITY_VIOLATION));
  int n = sample.size();
  double alpha = priorDistribution.GetAlpha();
  double beta = priorDistribution.GetBeta();
  BetaRand<> posteriorDistribution(alpha + r * n,
                                   beta + this->GetSampleSum(sample));
  SetParameters(r, MAP ? posteriorDistribution.Mode()
...
String NegativeBinomialRand<IntType, T>::Name() const {
  if (std::is_integral_v<T>)
    return "Pascal(" + this->toStringWithPrecision(this->GetNumber()) + ", " +
           this->toStringWithPrecision(this->GetProbability()) + ")";
  return "Polya(" + this->toStringWithPrecision(this->GetNumber()) + ", " +
         this->toStringWithPrecision(this->GetProbability()) + ")";
}
...
void NegativeBinomialRand<IntType, T>::Fit(const std::vector<IntType> &sample) {
  ...
  if (variance <= mean)
    throw std::invalid_argument(this->fitErrorDescription(
        this->NOT_APPLICABLE, this->TOO_SMALL_VARIANCE));
  ...
  if (!RandMath::findRootNewtonFirstOrder<double>(
    ...
    throw std::runtime_error(this->fitErrorDescription(
        this->UNDEFINED_ERROR, "Error in root-finding algorithm"));
  if (guess <= 0.0)
    throw std::runtime_error(this->fitErrorDescription(
        this->WRONG_RETURN,
        "Number should be positive, but returned value is " +
            this->toStringWithPrecision(guess)));
