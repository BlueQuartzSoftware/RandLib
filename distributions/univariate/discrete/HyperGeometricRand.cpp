...
HyperGeometricRand<IntType>::HyperGeometricRand(IntType totalSize,
                                                IntType drawsNum,
                                                IntType successesNum) {
  SetParameters(totalSize, drawsNum, successesNum);
}

template <typename IntType> String HyperGeometricRand<IntType>::Name() const {
...
template <typename IntType>
void HyperGeometricRand<IntType>::SetParameters(IntType totalSize,
                                                IntType drawsNum,
                                                IntType successesNum) {
  if (totalSize <= 0 || drawsNum <= 0 || successesNum <= 0)
    throw std::invalid_argument(
        "HyperGeometric distribution: all parameters should be positive");
...
template <typename IntType> IntType HyperGeometricRand<IntType>::Mode() const {
  double mode = (n + 1) * (K + 1);
