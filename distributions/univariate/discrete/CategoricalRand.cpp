...
template <typename IntType> String CategoricalRand<IntType>::Name() const {
...
template <typename IntType>
void CategoricalRand<IntType>::SetProbabilities(
    std::vector<double> &&probabilities) {
  if (probabilities.size() == 0 ||
      std::accumulate(probabilities.begin(), probabilities.end(), 0.0) != 1.0)
    throw std::invalid_argument(
        "Categorical distribution: probability parameters should sum to 1");
  else {
...
template <typename IntType> IntType CategoricalRand<IntType>::Variate() const {
  double U = UniformRand<double>::StandardVariate(this->localRandGenerator);
  return quantileImpl(U);
}

template <typename IntType> long double CategoricalRand<IntType>::Mean() const {
  long double sum = 0.0;
  for (int i = 1; i != K; ++i)
    sum += i * prob[i];
  return sum;
...
template <typename IntType> IntType CategoricalRand<IntType>::Mode() const {
