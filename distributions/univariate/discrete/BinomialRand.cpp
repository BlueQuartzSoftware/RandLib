...
BinomialDistribution<IntType>::BinomialDistribution(IntType number,
                                                    double probability) {
...
template <typename IntType>
void BinomialDistribution<IntType>::SetParameters(IntType number,
                                                  double probability) {
  ...
  if (number <= 0)
    throw std::invalid_argument(
        "Binomial distribution: number should be positive");
  n = number;
  p = probability;
  q = 1.0 - p;
  np = n * p;
...
template <typename IntType>
double
BinomialDistribution<IntType>::SourceToNatural(double sourceParameters) const {
...
template <typename IntType>
double
BinomialDistribution<IntType>::LogNormalizerGradient(double theta) const {
...
IntType BinomialDistribution<IntType>::variateRejection() const {
  ...
  do {
    float U =
        a4 * UniformRand<float>::StandardVariate(this->localRandGenerator);
    if (U <= a1) {
      ...
      if (!reject) {
        float W =
            ExponentialRand<float>::StandardVariate(this->localRandGenerator);
        X = std::floor(Y);
        V = -W - 0.5 * N * N + c;
      }
    } else if (U <= a2) {
...
template <typename IntType>
IntType BinomialDistribution<IntType>::variateWaiting(
    IntType number, double probability, RandGenerator &randGenerator) {
  IntType X = -1;
  double sum = 0;
  do {
    IntType add =
        GeometricRand<IntType>::Variate(probability, randGenerator) + 1;
...
template <typename IntType>
IntType BinomialDistribution<IntType>::variateBernoulliSum(
    IntType number, double probability, RandGenerator &randGenerator) {
  IntType var = 0;
...
template <typename IntType>
IntType BinomialDistribution<IntType>::Variate(IntType number,
                                               double probability,
                                               RandGenerator &randGenerator) {
  /// sanity check
  if (number < 0)
    throw std::invalid_argument(
        "Binomial distribution: number should be positive, but it's equal to " +
        std::to_string(number));
  if (probability < 0.0 || probability > 1.0)
    throw std::invalid_argument(
        "Binomial distribution: probability parameter should in interval [0, "
        "1], but it's equal to " +
...
template <typename IntType>
void BinomialDistribution<IntType>::Sample(
...
template <typename IntType>
void BinomialDistribution<IntType>::FitProbability(
  ...
  if (!this->allElementsAreNotGreaterThan(n, sample))
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE,
        this->UPPER_LIMIT_VIOLATION + this->toStringWithPrecision(n)));
  SetParameters(n, this->GetSampleMean(sample) / n);
}

template <typename IntType>
BetaRand<> BinomialDistribution<IntType>::FitProbabilityBayes(
    const std::vector<IntType> &sample,
  ...
  SetParameters(n, MAP ? posteriorDistribution.Mode()
                       : posteriorDistribution.Mean());
  return posteriorDistribution;
}

template <typename IntType>
BetaRand<> BinomialDistribution<IntType>::FitProbabilityMinimax(
    const std::vector<IntType> &sample) {
...
template <typename IntType> String BinomialRand<IntType>::Name() const {
  return "Binomial(" + this->toStringWithPrecision(this->GetNumber()) + ", " +
