...
template <typename IntType> String PoissonRand<IntType>::Name() const {
  return "Poisson(" + this->toStringWithPrecision(GetRate()) + ")";
}

template <typename IntType> void PoissonRand<IntType>::SetGeneratorConstants() {
...
template <typename IntType> void PoissonRand<IntType>::SetRate(double rate) {
  if (rate <= 0.0)
    throw std::invalid_argument(
        "Poisson distribution: rate should be positive, but it's equal to " +
        std::to_string(rate));
  lambda = rate;
...
IntType PoissonRand<IntType>::variateRejection() const {
  ...
  do {
    ...
    if (!reject &&
        W - ExponentialRand<float>::StandardVariate(this->localRandGenerator) <=
            acceptanceFunction(X)) {
      return X + mu;
    }
...
template <typename IntType> IntType PoissonRand<IntType>::Variate() const {
  return generateByInversion() ? variateInversion() : variateRejection();
}

template <typename IntType>
IntType PoissonRand<IntType>::Variate(double rate,
                                      RandGenerator &randGenerator) {
  /// check validness of parameter
  if (rate <= 0.0)
    throw std::invalid_argument(
        "Poisson distribution: rate should be positive");
  if (rate > 1000) {
...
template <typename IntType> long double PoissonRand<IntType>::Mean() const {
  return lambda;
}

template <typename IntType> long double PoissonRand<IntType>::Variance() const {
  return lambda;
}
...
template <typename IntType> IntType PoissonRand<IntType>::Median() const {
  /// this value is approximate
  return std::max(std::floor(lambda + 1.0 / 3 - 0.02 / lambda), 0.0);
}

template <typename IntType> IntType PoissonRand<IntType>::Mode() const {
...
template <typename IntType> long double PoissonRand<IntType>::Skewness() const {
...
void PoissonRand<IntType>::Fit(const std::vector<IntType> &sample,
  ...
  if (significanceLevel <= 0 || significanceLevel > 1)
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_LEVEL,
        "Alpha is equal to " + this->toStringWithPrecision(significanceLevel)));
...
template <typename IntType>
GammaRand<>
PoissonRand<IntType>::FitBayes(const std::vector<IntType> &sample,
  ...
  GammaRand<> posteriorDistribution(alpha + this->GetSampleSum(sample),
                                    beta + sample.size());
