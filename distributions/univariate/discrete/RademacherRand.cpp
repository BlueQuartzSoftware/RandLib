...
String RademacherRand::Name() const { return "Rademacher"; }
...
long double RademacherRand::Mean() const { return 0; }

long double RademacherRand::Variance() const { return 1; }

std::complex<double> RademacherRand::CFImpl(double t) const {
  return std::cos(t);
}

int RademacherRand::Median() const { return -1; }
...
long double RademacherRand::Skewness() const { return 0.0; }

long double RademacherRand::ExcessKurtosis() const { return -2.0; }

int RademacherRand::quantileImpl(double p) const { return (p <= 0.5) ? -1 : 1; }

int RademacherRand::quantileImpl1m(double p) const {
  return (p >= 0.5) ? -1 : 1;
}

double RademacherRand::Entropy() { return M_LN2; }
