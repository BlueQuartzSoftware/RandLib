...
template <typename IntType> IntType GeometricRand<IntType>::Variate() const {
  typename PascalRand<IntType>::GENERATOR_ID genId =
      this->GetIdOfUsedGenerator();
...
template <typename IntType>
IntType GeometricRand<IntType>::Variate(double probability,
                                        RandGenerator &randGenerator) {
  if (probability > 1.0 || probability < 0.0)
    throw std::invalid_argument(
        "Geometric distribution: probability parameter should be in interval "
...
template <typename IntType> IntType GeometricRand<IntType>::Median() const {
  double median = -M_LN2 / this->log1mProb;
  double flooredMedian = std::floor(median);
  return (RandMath::areClose(median, flooredMedian, 1e-8)) ? flooredMedian - 1
                                                           : flooredMedian;
}
