...
UniformDiscreteRand<IntType>::UniformDiscreteRand(IntType minValue,
                                                  IntType maxValue) {
  SetBoundaries(minValue, maxValue);
}

template <typename IntType> String UniformDiscreteRand<IntType>::Name() const {
  return "Uniform Discrete(" + this->toStringWithPrecision(MinValue()) + ", " +
         this->toStringWithPrecision(MaxValue()) + ")";
}

template <typename IntType>
void UniformDiscreteRand<IntType>::SetBoundaries(IntType minValue,
                                                 IntType maxValue) {
  if (minValue > maxValue)
    throw std::invalid_argument("Uniform discrete distribution: minimum value "
                                "shouldn't be greater than maximum value");
...
template <typename IntType>
IntType UniformDiscreteRand<IntType>::StandardVariate(
    IntType minValue, IntType maxValue, RandGenerator &randGenerator) {
...
template <typename IntType> IntType UniformDiscreteRand<IntType>::Mode() const {
...
template <typename IntType>
double UniformDiscreteRand<IntType>::LikelihoodFunction(
    const std::vector<IntType> &sample) const {
  bool sampleIsInsideInterval = this->allElementsAreNotSmallerThan(a, sample) &&
                                this->allElementsAreNotGreaterThan(b, sample);
  return sampleIsInsideInterval ? std::pow(n, -sample.size()) : 0.0;
}

template <typename IntType>
double UniformDiscreteRand<IntType>::LogLikelihoodFunction(
    const std::vector<IntType> &sample) const {
  bool sampleIsInsideInterval = this->allElementsAreNotSmallerThan(a, sample) &&
                                this->allElementsAreNotGreaterThan(b, sample);
