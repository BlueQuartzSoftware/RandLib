...
NegativeHyperGeometricRand<IntType>::NegativeHyperGeometricRand(
    IntType totalSize, IntType totalSuccessesNum, IntType limitSuccessesNum) {
...
template <typename IntType>
void NegativeHyperGeometricRand<IntType>::SetParameters(
    IntType totalSize, IntType totalSuccessesNum, IntType limitSuccessesNum) {
  ...
  if (totalSuccessesNum > totalSize)
    throw std::invalid_argument(
        "Negative-HyperGeometric distribution: total size shouldn't be smaller "
        "than total successes number");
  if (limitSuccessesNum > totalSuccessesNum)
    throw std::invalid_argument(
        "Negative-HyperGeometric distribution: total successes number "
...
IntType NegativeHyperGeometricRand<IntType>::Variate() const {
  ...
  while (successesNum < m) {
    ++num;
    if (BernoulliRand::Variate(p, this->localRandGenerator) &&
        ++successesNum == num - N + M)
      return N - M;
    p = M - successesNum;
    p /= N - num;
  }
