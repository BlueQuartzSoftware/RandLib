...
template <typename IntType> String SkellamRand<IntType>::Name() const {
  return "Skellam(" + this->toStringWithPrecision(GetFirstRate()) + ", " +
         this->toStringWithPrecision(GetSecondRate()) + ")";
}
...
void SkellamRand<IntType>::SetRates(double rate1, double rate2) {
  if (rate1 <= 0.0 || rate2 <= 0.0)
    throw std::invalid_argument(
        "Skellam distribution: rates should be positive");
...
double SkellamRand<IntType>::F(const IntType &k) const {
  return (k < 0)
             ? RandMath::MarcumP(-k, mu1, mu2, sqrtMu1, sqrtMu2, logMu1, logMu2)
             : RandMath::MarcumQ(k + 1, mu2, mu1, sqrtMu2, sqrtMu1, logMu2,
                                 logMu1);
}
...
double SkellamRand<IntType>::S(const IntType &k) const {
  return (k < 0)
             ? RandMath::MarcumQ(-k, mu1, mu2, sqrtMu1, sqrtMu2, logMu1, logMu2)
             : RandMath::MarcumP(k + 1, mu2, mu1, sqrtMu2, sqrtMu1, logMu2,
                                 logMu1);
}

template <typename IntType> IntType SkellamRand<IntType>::Variate() const {
...
template <typename IntType> long double SkellamRand<IntType>::Mean() const {
  return mu1 - mu2;
}

template <typename IntType> long double SkellamRand<IntType>::Variance() const {
  return mu1 + mu2;
}

template <typename IntType> IntType SkellamRand<IntType>::Median() const {
  return distributions::DiscreteDistribution<IntType>::quantileImpl(0.5,
                                                                    mu1 - mu2);
}
...
template <typename IntType> IntType SkellamRand<IntType>::Mode() const {
  return Mean();
}

template <typename IntType> long double SkellamRand<IntType>::Skewness() const {
