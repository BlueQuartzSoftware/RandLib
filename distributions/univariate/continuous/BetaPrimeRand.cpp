...
template <typename RealType> String BetaPrimeRand<RealType>::Name() const {
  return "Beta Prime(" + this->toStringWithPrecision(GetAlpha()) + ", " +
...
void BetaPrimeRand<RealType>::SetShapes(double shape1, double shape2) {
  if (shape1 <= 0 || shape2 <= 0)
    throw std::invalid_argument(
        "Beta-prime distribution: shapes should be positive");
...
template <typename RealType>
RealType
BetaPrimeRand<RealType>::fromBetaVariate(const RealType &betaVar) const {
  if (betaVar > 1e-5)
...
template <typename RealType> RealType BetaPrimeRand<RealType>::Variate() const {
  double x = B.Variate();
...
template <typename RealType> long double BetaPrimeRand<RealType>::Mean() const {
...
template <typename RealType> RealType BetaPrimeRand<RealType>::Median() const {
  return (alpha == beta) ? 1.0 : quantileImpl(0.5);
}

template <typename RealType> RealType BetaPrimeRand<RealType>::Mode() const {
...
long double BetaPrimeRand<RealType>::ExcessKurtosis() const {
  ...
  long double numerator =
      betam1 * betam1 * (beta - 2) / (alpha * (alpha + betam1));
  numerator += 5 * beta - 11;
...
void BetaPrimeRand<RealType>::FitAlpha(const std::vector<RealType> &sample) {
  ...
  long double lnG =
      this->GetSampleLogMean(sample) - B.GetSampleLog1pMeanNorm(sample);
