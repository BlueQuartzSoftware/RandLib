...
BetaDistribution<RealType>::BetaDistribution(double shape1, double shape2,
...
double BetaDistribution<RealType>::F(const RealType &x) const {
  ...
  return RandMath::ibeta(xSt, alpha, beta, logBetaFun, std::log(xSt),
                         std::log1pl(-xSt));
}
...
double BetaDistribution<RealType>::S(const RealType &x) const {
  ...
  return RandMath::ibeta(1.0 - xSt, beta, alpha, logBetaFun, std::log1pl(-xSt),
                         std::log(xSt));
}
...
RealType BetaDistribution<RealType>::variateArcsine() const {
  RealType U =
      2 * UniformRand<RealType>::StandardVariate(this->localRandGenerator) - 1;
...
RealType BetaDistribution<RealType>::variateRejectionUniform() const {
  ...
  do {
    RealType U =
        UniformRand<RealType>::StandardVariate(this->localRandGenerator);
    RealType V =
        UniformRand<RealType>::StandardVariate(this->localRandGenerator);
    if (0.25 * V * V <= U - U * U)
      return U;
...
RealType BetaDistribution<RealType>::variateJohnk() const {
  ...
  RealType W = 0, V = 0;
  do {
    W = ExponentialRand<RealType>::StandardVariate(this->localRandGenerator) /
        alpha;
    V = ExponentialRand<RealType>::StandardVariate(this->localRandGenerator) /
        beta;
    X = std::exp(-W);
    Z = X + std::exp(-V);
  } while (Z > 1);
...
RealType BetaDistribution<RealType>::variateAtkinsonWhittaker() const {
  ...
  do {
    ...
    if (U <= genCoef.s) {
      ...
      RealType X =
          1.0 - (1.0 - genCoef.t) *
                    std::pow((1.0 - U) / (1.0 - genCoef.s), 1.0 / beta);
...
template <typename RealType>
void BetaDistribution<RealType>::Sample(
...
RealType BetaDistribution<RealType>::Median() const {
  ...
  if (alpha >= 1.0 && beta >= 1.0) {
    ...
    initValue *= bma;
    initValue += a;
    return distributions::ContinuousDistribution<RealType>::quantileImpl(
        0.5, initValue);
  }
  return distributions::ContinuousDistribution<RealType>::quantileImpl(0.5);
}

template <typename RealType> RealType BetaDistribution<RealType>::Mode() const {
  double mode;
  if (alpha > 1)
...
std::complex<double> BetaDistribution<RealType>::CFImpl(double t) const {
  ...
  if (alpha >= 1 && beta >= 1)
    return distributions::UnivariateDistribution<RealType>::CFImpl(t);
...
template <typename RealType> String BetaRand<RealType>::Name() const {
...
template <typename RealType>
DoublePair
BetaRand<RealType>::SourceToNatural(DoublePair sourceParameters) const {
...
template <typename RealType>
long double BetaRand<RealType>::GetSampleLogMeanNorm(
...
template <typename RealType>
long double BetaRand<RealType>::GetSampleLog1pMeanNorm(
    const std::vector<RealType> &sample) const {
  long double lnG1p = 0;
  for (RealType var : sample) {
    RealType x = (var - this->a) * this->bmaInv;
...
template <typename RealType>
long double BetaRand<RealType>::GetSampleLog1mMeanNorm(
    const std::vector<RealType> &sample) const {
  long double lnG1m = 0;
  for (RealType var : sample) {
    RealType x = (var - this->a) * this->bmaInv;
...
void BetaRand<RealType>::FitAlpha(long double lnG, long double meanNorm) {
  if (meanNorm <= 0 || meanNorm >= 1)
    throw std::invalid_argument(this->fitErrorDescription(
        this->NOT_APPLICABLE,
        "Normalized mean of the sample should be in interval of (0, 1)"));
  if (this->beta == 1.0) {
    ...
    if (!RandMath::findRootNewtonFirstOrder<RealType>(
            [this, lnG](double x) {
              double first = RandMath::digamma(x) -
                             RandMath::digamma(x + this->beta) - lnG;
...
void BetaRand<RealType>::FitAlpha(const std::vector<RealType> &sample) {
  ...
  if (!std::isfinite(lnG))
    throw std::runtime_error(
        this->fitErrorDescription(this->WRONG_RETURN, this->ALPHA_ZERO));
...
void BetaRand<RealType>::FitBeta(long double lnG1m, long double meanNorm) {
  ...
  if (this->alpha == 1.0) {
    ...
    if (!RandMath::findRootNewtonFirstOrder<RealType>(
            [this, lnG1m](double x) {
              double first = RandMath::digamma(x) -
                             RandMath::digamma(x + this->alpha) - lnG1m;
              double second = RandMath::trigamma(x);
              return DoublePair(first, second);
            },
            shape))
...
void BetaRand<RealType>::FitBeta(const std::vector<RealType> &sample) {
  ...
  if (!std::isfinite(lnG1m))
    throw std::runtime_error(
        this->fitErrorDescription(this->WRONG_RETURN, this->BETA_ZERO));
  long double mean = this->GetSampleMean(sample);
  mean -= this->a;
  mean *= this->bmaInv;
  FitBeta(lnG1m, mean);
...
template <typename RealType>
void BetaRand<RealType>::FitShapes(long double lnG, long double lnG1m,
  ...
  if (!RandMath::findRootNewtonFirstOrder2d(
          ...
          [](DoublePair x) {
            ...
            DoublePair first = std::make_pair(
                trigammaAlphapBeta - trigammaAlpha, trigammaAlphapBeta);
            DoublePair second = std::make_pair(
                trigammaAlphapBeta, trigammaAlphapBeta - trigammaBeta);
...
template <typename RealType> String ArcsineRand<RealType>::Name() const {
...
template <typename RealType>
BaldingNicholsRand<RealType>::BaldingNicholsRand(double fixatingIndex,
                                                 double frequency) {
  SetFixatingIndexAndFrequency(fixatingIndex, frequency);
}

template <typename RealType> String BaldingNicholsRand<RealType>::Name() const {
  return "Balding-Nichols(" + this->toStringWithPrecision(GetFixatingIndex()) +
         ", " + this->toStringWithPrecision(GetFrequency()) + ")";
}

template <typename RealType>
void BaldingNicholsRand<RealType>::SetFixatingIndexAndFrequency(
    double fixatingIndex, double frequency) {
  F = fixatingIndex;
  if (F <= 0 || F >= 1)
    F = 0.5;
