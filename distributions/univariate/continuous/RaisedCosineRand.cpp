...
RaisedCosineDistribution<RealType>::RaisedCosineDistribution(double location,
                                                             double scale) {
...
void RaisedCosineDistribution<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Raised-Cosine distribution: scale should be positive");
  s = scale;
  s_pi = s * M_1_PI;
  log2S = std::log(2 * s);
}
...
template <typename RealType>
RealType RaisedCosineDistribution<RealType>::StandardVariate(
    RandGenerator &randGenerator) {
  /// p. 160. Non-Uniform Random Variate Generation. Luc Devroye
  RealType X =
      M_PI * UniformRand<RealType>::StandardVariate(randGenerator) - M_PI_2;
  RealType XSq = X * X;
...
template <typename RealType>
std::complex<double>
RaisedCosineDistribution<RealType>::CFImpl(double t) const {
  double st = s * t;
...
template <typename RealType> String RaisedCosineRand<RealType>::Name() const {
  return "Raised cosine(" + this->toStringWithPrecision(this->GetLocation()) +
...
template <typename RealType> String RaabGreenRand<RealType>::Name() const {
  return "Raab Green";
}
