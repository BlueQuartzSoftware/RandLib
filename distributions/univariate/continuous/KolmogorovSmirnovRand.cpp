...
RealType KolmogorovSmirnovRand<RealType>::truncatedGammaVariate() const {
  ...
  size_t iter = 0;
  do {
    RealType E0 = rate * ExponentialRand<RealType>::StandardVariate(
                             this->localRandGenerator);
    RealType E1 = 2 * ExponentialRand<RealType>::StandardVariate(
                          this->localRandGenerator);
    RealType G = tp + E0;
    if (E0 * E0 <= tp * E1 * (G + tp))
      return G;
...
template <typename RealType>
RealType
KolmogorovSmirnovRand<RealType>::variateForTheLeftMostInterval() const {
  size_t iter1 = 0;
  do {
    ...
    while (U >= W &&
           ++iter2 <= ProbabilityDistribution<RealType>::MAX_ITER_REJECTION) {
      W += Z * Q;
      if (U >= W)
        return X;
      n += 2;
...
template <typename RealType>
RealType
KolmogorovSmirnovRand<RealType>::variateForTheRightMostInterval() const {
  static constexpr double tSq =
      0.5625; /// square of parameter t suggested in the book
  size_t iter1 = 0;
  do {
    RealType E =
...
RealType KolmogorovSmirnovRand<RealType>::Variate() const {
  ...
  bool isLeft = UniformRand<RealType>::StandardVariate(
                    this->localRandGenerator) < 0.3728329582237386; /// F(0.75)
  return isLeft ? variateForTheLeftMostInterval()
                : variateForTheRightMostInterval();
}
  ...
  if (p < 1e-5) {
    ...
    if (!RandMath::findRootNewtonFirstOrder<RealType>(
      ...
      throw std::runtime_error(
          "Kolmogorov-Smirnov distribution: failure in numerical procedure");
    return guess;
  }
  if (!RandMath::findRootNewtonFirstOrder<RealType>(
          [p, this](RealType x) {
