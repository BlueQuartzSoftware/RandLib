...
template <typename RealType> String LevyRand<RealType>::Name() const {
  return "Levy(" + this->toStringWithPrecision(this->GetLocation()) + ", " +
...
template <typename RealType> RealType LevyRand<RealType>::Variate() const {
...
void LevyRand<RealType>::FitScale(const std::vector<RealType> &sample) {
  ...
  if (!this->allElementsAreNotSmallerThan(this->mu, sample))
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE,
        this->LOWER_LIMIT_VIOLATION + this->toStringWithPrecision(this->mu)));
