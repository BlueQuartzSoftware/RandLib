...
StudentTRand<RealType>::StudentTRand(double degree, double location,
                                     double scale) {
  SetDegree(degree);
...
template <typename RealType> String StudentTRand<RealType>::Name() const {
  if (mu == 0.0 && sigma == 1.0)
...
void StudentTRand<RealType>::SetDegree(double degree) {
  if (degree <= 0.0)
    throw std::invalid_argument(
        "Student-t distribution: degree parameter should be positive");
  nu = degree;
...
void StudentTRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Student-t distribution: scale should be positive");
  sigma = scale;
...
double StudentTRand<RealType>::F(const RealType &x) const {
  ...
  double t = nu / (xSq + nu);
  double y = 0.5 * RandMath::ibeta(t, 0.5 * nu, 0.5, logBetaFun, std::log(t),
                                   std::log1pl(-t));
...
template <typename RealType> RealType StudentTRand<RealType>::Variate() const {
  if (nu == 1)
    return mu + sigma * CauchyRand<RealType>::StandardVariate(
...
void StudentTRand<RealType>::Sample(std::vector<RealType> &outputData) const {
  if (nu == 1) {
    for (RealType &var : outputData)
      var = mu + sigma * CauchyRand<RealType>::StandardVariate(
                             this->localRandGenerator);
  } else {
    Y.Sample(outputData);
    for (RealType &var : outputData)
      var = mu + sigma *
                     NormalRand<RealType>::StandardVariate(
                         this->localRandGenerator) /
                     var;
  }
}
...
template <typename RealType> long double StudentTRand<RealType>::Mean() const {
  return (nu > 1) ? mu : NAN;
}
...
template <typename RealType> RealType StudentTRand<RealType>::Median() const {
  return mu;
}

template <typename RealType> RealType StudentTRand<RealType>::Mode() const {
  return mu;
}
