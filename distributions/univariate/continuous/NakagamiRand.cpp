...
NakagamiDistribution<RealType>::NakagamiDistribution(double shape,
                                                     double spread) {
  SetParameters(shape, spread);
}

template <typename RealType>
void NakagamiDistribution<RealType>::SetParameters(double shape,
                                                   double spread) {
  if (shape <= 0.0)
    throw std::invalid_argument(
        "Nakagami distribution: shape should be positive");
  if (spread <= 0.0)
    throw std::invalid_argument(
        "Nakagami distribution: spread should be positive");
  mu = shape;
  omega = spread;
...
template <typename RealType>
void NakagamiDistribution<RealType>::Sample(
...
long double NakagamiDistribution<RealType>::Skewness() const {
  ...
  return (thirdMoment - mean * (3 * variance + mean * mean)) /
...
std::complex<double> NakagamiDistribution<RealType>::CFImpl(double t) const {
  ...
  double im = this->ExpectedValue(
      [this, t](double x) { return std::sin(t * x); }, 0, INFINITY);

  return std::complex<double>(re, im);
}

/// NAKAGAMI
template <typename RealType> String NakagamiRand<RealType>::Name() const {
  return "Nakagami(" + this->toStringWithPrecision(this->GetShape()) + ", " +
         this->toStringWithPrecision(this->GetSpread()) + ")";
}
...
/// CHI
template <typename RealType> ChiRand<RealType>::ChiRand(int degree) {
  SetDegree(degree);
}

template <typename RealType> String ChiRand<RealType>::Name() const {
  return "Chi(" + this->toStringWithPrecision(GetDegree()) + ")";
}

template <typename RealType> void ChiRand<RealType>::SetDegree(int degree) {
  if (degree < 1)
    throw std::invalid_argument(
        "Chi distribution: degree parameter should be positive");
  NakagamiDistribution<RealType>::SetParameters(0.5 * degree, degree);
}

template <typename RealType> long double ChiRand<RealType>::Skewness() const {
...
void MaxwellBoltzmannRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Maxwell-Boltzmann distribution: scale should be positive");
  sigma = scale;
...
template <typename RealType>
void MaxwellBoltzmannRand<RealType>::Sample(
    std::vector<RealType> &outputData) const {
  for (RealType &var : outputData)
    var = this->Variate();
}
...
template <typename RealType> String RayleighRand<RealType>::Name() const {
...
void RayleighRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Rayleigh distribution: scale should be positive");
  sigma = scale;
...
template <typename RealType> RealType RayleighRand<RealType>::Variate() const {
  double W =
      ExponentialRand<RealType>::StandardVariate(this->localRandGenerator);
...
template <typename RealType> long double RayleighRand<RealType>::Mean() const {
...
template <typename RealType> RealType RayleighRand<RealType>::Median() const {
  static constexpr double medianCoef = nonstd::sqrt(M_LN2 + M_LN2);
  return sigma * medianCoef;
}

template <typename RealType> RealType RayleighRand<RealType>::Mode() const {
  return sigma;
}

template <typename RealType>
long double RayleighRand<RealType>::Skewness() const {
  static constexpr long double skewness =
      2 * M_SQRTPI * (M_PI - 3) / nonstd::pow(4.0 - M_PI, 1.5);
  return skewness;
}
...
long double RayleighRand<RealType>::ExcessKurtosis() const {
  ...
  static constexpr long double kurtosis =
      -(6 * M_PI * M_PI - 24 * M_PI + 16) / (temp * temp);
  return kurtosis;
}

template <typename RealType>
void RayleighRand<RealType>::Fit(const std::vector<RealType> &sample,
                                 bool unbiased) {
  /// Sanity check
  ...
  /// Calculate unbiased sigma
    ...
    coef += 0.125 / n;
    SetScale(std::exp(std::log1pl(coef) +
                      0.5 * sigmaBiasedSq)); /// err ~ o(n^{-6.5}) < 1e-13
  } else if (n > 15) {
