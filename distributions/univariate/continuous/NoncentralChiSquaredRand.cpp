...
NoncentralChiSquaredRand<RealType>::NoncentralChiSquaredRand(
    double degree, double noncentrality) {
...
template <typename RealType>
void NoncentralChiSquaredRand<RealType>::SetParameters(double degree,
...
double NoncentralChiSquaredRand<RealType>::F(const RealType &x) const {
  ...
  return RandMath::MarcumP(halfK, halfLambda, halfX, sqrtLambda / M_SQRT2,
                           sqrtHalfX, logLambda - M_LN2, logHalfX);
}
...
double NoncentralChiSquaredRand<RealType>::S(const RealType &x) const {
  ...
  double sqrtHalfX = std::sqrt(halfX);
  double logHalfX = std::log(halfX);
  return RandMath::MarcumQ(halfK, halfLambda, halfX, sqrtLambda / M_SQRT2,
                           sqrtHalfX, logLambda - M_LN2, logHalfX);
}

template <typename RealType>
RealType NoncentralChiSquaredRand<RealType>::variateForDegreeEqualOne() const {
  RealType y = sqrtLambda +
...
template <typename RealType>
RealType
NoncentralChiSquaredRand<RealType>::Variate(double degree, double noncentrality,
  ...
  if (degree >= 1) {
    RealType rv = (degree == 1)
                      ? 0.0
                      : 2 * GammaDistribution<RealType>::StandardVariate(
                                0.5 * degree - 0.5, randGenerator);
    RealType y = std::sqrt(noncentrality) +
                 NormalRand<RealType>::StandardVariate(randGenerator);
    return rv + y * y;
  }
  RealType shape = 0.5 * degree + PoissonRand<int>::Variate(0.5 * noncentrality,
                                                            randGenerator);
...
RealType NoncentralChiSquaredRand<RealType>::Variate() const {
  if (k < 1)
    return 2 * GammaDistribution<RealType>::StandardVariate(
                   halfK + Y.Variate(), this->localRandGenerator);
  double X = variateForDegreeEqualOne();
  if (k > 1)
    X += 2 * GammaDistribution<RealType>::StandardVariate(
                 halfK - 0.5, this->localRandGenerator);
  return X;
}

template <typename RealType>
void NoncentralChiSquaredRand<RealType>::Sample(
  ...
  if (k >= 1) {
    ...
    if (halfKmHalf == 0)
      return;
    for (RealType &var : outputData)
      var += 2 * GammaDistribution<RealType>::StandardVariate(
                     halfKmHalf, this->localRandGenerator);
  } else {
    for (RealType &var : outputData)
      var = 2 * GammaDistribution<RealType>::StandardVariate(
                    halfK + Y.Variate(), this->localRandGenerator);
...
RealType NoncentralChiSquaredRand<RealType>::Mode() const {
  return (k <= 2) ? 0.0
                  : distributions::ContinuousDistribution<RealType>::Mode();
}
...
template <typename RealType>
std::complex<double>
NoncentralChiSquaredRand<RealType>::CFImpl(double t) const {
