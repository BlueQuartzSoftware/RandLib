...
template <typename RealType> String ParetoRand<RealType>::Name() const {
  return "Pareto(" + this->toStringWithPrecision(GetShape()) + ", " +
         this->toStringWithPrecision(GetScale()) + ")";
}

template <typename RealType> void ParetoRand<RealType>::SetShape(double shape) {
  if (shape <= 0.0)
    throw std::invalid_argument(
        "Pareto distribution: shape should be positive");
  alpha = shape;
  logAlpha = std::log(alpha);
}

template <typename RealType> void ParetoRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Pareto distribution: scale should be positive");
  sigma = scale;
...
template <typename RealType>
RealType
ParetoRand<RealType>::variateForAlphaEqualOne(RandGenerator &randGenerator) {
  return 1.0 / UniformRand<RealType>::StandardVariate(randGenerator);
}

template <typename RealType>
RealType
ParetoRand<RealType>::variateForAlphaEqualTwo(RandGenerator &randGenerator) {
  return 1.0 / std::sqrt(UniformRand<RealType>::StandardVariate(randGenerator));
}

template <typename RealType>
RealType
ParetoRand<RealType>::variateForGeneralAlpha(double shape,
                                             RandGenerator &randGenerator) {
  return std::exp(ExponentialRand<RealType>::StandardVariate(randGenerator) /
                  shape);
}

template <typename RealType> RealType ParetoRand<RealType>::Variate() const {
  return sigma * StandardVariate(alpha, this->localRandGenerator);
}

template <typename RealType>
RealType ParetoRand<RealType>::StandardVariate(double shape,
                                               RandGenerator &randGenerator) {
...
template <typename RealType> long double ParetoRand<RealType>::Mean() const {
...
template <typename RealType> RealType ParetoRand<RealType>::Median() const {
  return std::exp(logSigma + M_LN2 / alpha);
}

template <typename RealType> RealType ParetoRand<RealType>::Mode() const {
  return sigma;
}
...
template <typename RealType> long double ParetoRand<RealType>::Entropy() const {
  return logSigma - logAlpha + 1.0 / alpha + 1;
}

template <typename RealType>
void ParetoRand<RealType>::FitShape(const std::vector<RealType> &sample,
                                    bool unbiased) {
  if (!this->allElementsAreNotSmallerThan(sigma, sample))
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE,
        this->LOWER_LIMIT_VIOLATION + this->toStringWithPrecision(sigma)));
  double invShape = this->GetSampleLogMean(sample) - logSigma;
  if (invShape == 0.0)
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE, "Possibly all the elements of the sample coincide "
...
template <typename RealType>
void ParetoRand<RealType>::FitScale(const std::vector<RealType> &sample,
                                    bool unbiased) {
  ...
  double scale =
      unbiased ? (1.0 - 1.0 / (sample.size() * alpha)) * minVar : minVar;
  SetScale(scale);
}

template <typename RealType>
void ParetoRand<RealType>::Fit(const std::vector<RealType> &sample,
                               bool unbiased) {
  RealType minVar = *std::min_element(sample.begin(), sample.end());
  if (minVar <= 0)
    throw std::invalid_argument(this->fitErrorDescription(
  ...
  if (invShape == 0.0)
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE,
        "Possibly all the elements of the sample are the same."));
  double shape = 1.0 / invShape;
...
template <typename RealType>
GammaRand<RealType> ParetoRand<RealType>::FitShapeBayes(
  ...
  double newRate = priorDistribution.GetRate() +
                   n * (this->GetSampleLogMean(sample) - logSigma);
