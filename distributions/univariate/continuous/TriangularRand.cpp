...
TriangularRand<RealType>::TriangularRand(double lowerLimit, double mode,
                                         double upperLimit) {
  SetParameters(lowerLimit, mode, upperLimit);
}

template <typename RealType> String TriangularRand<RealType>::Name() const {
  return "Triangular(" + this->toStringWithPrecision(MinValue()) + ", " +
         this->toStringWithPrecision(Mode()) + ", " +
         this->toStringWithPrecision(MaxValue()) + ")";
}

template <typename RealType>
void TriangularRand<RealType>::SetParameters(double lowerLimit, double mode,
                                             double upperLimit) {
  if (lowerLimit >= mode)
    throw std::invalid_argument(
        "Triangular distribution: lower limit should be greater than mode");
  if (mode >= upperLimit)
    throw std::invalid_argument(
        "Triangular distribution: upper limit should be smaller than mode");
  a = lowerLimit;
  c = mode;
  b = upperLimit;
...
template <typename RealType> RealType TriangularRand<RealType>::Median() const {
  if (c + c > a + b)
    return a + std::sqrt(0.5 * coefGenerator1);
  return b - std::sqrt(0.5 * coefGenerator2);
}

template <typename RealType> RealType TriangularRand<RealType>::Mode() const {
  return c;
}
