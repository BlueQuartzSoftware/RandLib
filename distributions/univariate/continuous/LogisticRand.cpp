...
template <typename RealType> String LogisticRand<RealType>::Name() const {
  return "Logistic(" + this->toStringWithPrecision(GetLocation()) + ", " +
...
void LogisticRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Logistic distribution: scale of should be positive");
  s = scale;
  logS = std::log(s);
}
...
template <typename RealType> RealType LogisticRand<RealType>::Variate() const {
  /// there can be used rejection method from Laplace or Cauchy (Luc Devroye, p.
  /// 471) or ziggurat
  return mu + s * std::log(1.0 / UniformRand<RealType>::StandardVariate(
                                     this->localRandGenerator) -
                           1);
}

template <typename RealType> long double LogisticRand<RealType>::Mean() const {
  return mu;
}
...
template <typename RealType> RealType LogisticRand<RealType>::Median() const {
  return mu;
}

template <typename RealType> RealType LogisticRand<RealType>::Mode() const {
  return mu;
}
