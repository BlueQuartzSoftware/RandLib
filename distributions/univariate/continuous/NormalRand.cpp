...
template <typename RealType> String NormalRand<RealType>::Name() const {
  return "Normal(" + this->toStringWithPrecision(this->GetLocation()) + ", " +
         this->toStringWithPrecision(this->Variance()) + ")";
}

template <typename RealType> void NormalRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Normal distribution: scale should be positive, but it's equal to " +
        std::to_string(scale));
  sigma = scale;
...
template <typename RealType>
DoublePair
NormalRand<RealType>::SourceToNatural(DoublePair sourceParameters) const {
...
template <typename RealType> RealType NormalRand<RealType>::Variate() const {
...
RealType NormalRand<RealType>::StandardVariate(RandGenerator &randGenerator) {
  ...
  do {
    ...
    int stairId = B & 255;
    RealType x = UniformRand<RealType>::StandardVariate(randGenerator) *
                 ziggurat[stairId].second; /// Get horizontal coordinate
    ...
    if (stairId == 0) /// handle the base layer
    {
      ...
      if (z <= 0) /// if previous generation wasn't successful
      {
        do {
          x = ExponentialRand<RealType>::StandardVariate(randGenerator) /
              ziggurat[1].second;
          z = ExponentialRand<RealType>::StandardVariate(randGenerator) -
              0.5 * x * x; /// we storage this value as after acceptance it
    ...
    if (ziggurat[stairId - 1].first +
            height * UniformRand<RealType>::StandardVariate(randGenerator) <
        std::exp(-.5 * x * x))
      return ((signed)B > 0) ? x : -x;
...
long double NormalRand<RealType>::Moment(size_t n) const {
  if (n == 0)
    return 1;
  return (n & 1) ? std::exp(n * this->GetLogScale() + RandMath::ldfact(n - 1))
                 : 0.0;
}
...
void NormalRand<RealType>::FitLocation(const std::vector<RealType> &sample,
  ...
  if (significanceLevel <= 0 || significanceLevel > 1)
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_LEVEL, "Input level is equal to " +
                               this->toStringWithPrecision(significanceLevel)));
...
void NormalRand<RealType>::Fit(const std::vector<RealType> &sample,
  ...
  if (unbiased) {
    ...
    if (n <= 1)
      throw std::invalid_argument(this->fitErrorDescription(
          this->TOO_FEW_ELEMENTS, "There should be at least 2 elements"));
...
void NormalRand<RealType>::Fit(const std::vector<RealType> &sample,
                               ...
                               DoublePair &confidenceIntervalForVariance,
                               double significanceLevel, bool unbiased) {
  if (significanceLevel <= 0 || significanceLevel > 1)
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_LEVEL, "Input level is equal to " +
                               this->toStringWithPrecision(significanceLevel)));
  ...
  confidenceIntervalForVariance.first =
      numerator / ChiSqRV.Quantile1m(halfAlpha);
  confidenceIntervalForVariance.second =
      numerator / ChiSqRV.Quantile(halfAlpha);
}

template <typename RealType>
NormalRand<RealType> NormalRand<RealType>::FitLocationBayes(
    const std::vector<RealType> &sample,
    const NormalRand<RealType> &priorDistribution, bool MAP) {
  ...
  NormalRand<RealType> posteriorDistribution(numerator / denominator,
                                             1.0 / denominator);
  this->SetLocation(MAP ? posteriorDistribution.Mode()
                        : posteriorDistribution.Mean());
  return posteriorDistribution;
}

template <typename RealType>
InverseGammaRand<RealType> NormalRand<RealType>::FitVarianceBayes(
    const std::vector<RealType> &sample,
  ...
  double betaPosterior =
      betaPrior + halfN * this->GetSampleVariance(sample, this->mu);
  InverseGammaRand<RealType> posteriorDistribution(alphaPosterior,
                                                   betaPosterior);
  SetVariance(MAP ? posteriorDistribution.Mode()
                  : posteriorDistribution.Mean());
  return posteriorDistribution;
}

template <typename RealType>
NormalInverseGammaRand<RealType> NormalRand<RealType>::FitBayes(
    const std::vector<RealType> &sample,
    const NormalInverseGammaRand<RealType> &priorDistribution, bool MAP) {
  size_t n = sample.size();
  double alphaPrior = priorDistribution.GetShape();
  double betaPrior = priorDistribution.GetRate();
  ...
  double muPosterior =
      (lambdaPrior * muPrior + n * stats.first) / lambdaPosterior;
  double halfN = 0.5 * n;
  double alphaPosterior = alphaPrior + halfN;
  double aux = muPrior - stats.first;
  double betaPosterior =
      betaPrior +
      halfN * (stats.second + lambdaPrior / lambdaPosterior * aux * aux);
  NormalInverseGammaRand<RealType> posteriorDistribution(
      muPosterior, lambdaPosterior, alphaPosterior, betaPosterior);
  DoublePair newParams =
      MAP ? static_cast<DoublePair>(posteriorDistribution.Mode())
          : static_cast<DoublePair>(posteriorDistribution.Mean());
