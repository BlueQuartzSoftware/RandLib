...
void WrappedExponentialRand<RealType>::SetRate(double rate) {
  if (lambda <= 0.0)
    throw std::invalid_argument(
        "Wrapped Exponential distribution: rate parameter should be positive");
  lambda = rate;
...
double WrappedExponentialRand<RealType>::logf(const RealType &x) const {
  return (x < 0 || x > 2 * M_PI) ? -INFINITY
                                 : logLambda - lambda * x - logpdfCoef;
}
...
double WrappedExponentialRand<RealType>::F(const RealType &x) const {
  if (x <= 0.0)
    return 0.0;
  return (x < 2 * M_PI) ? std::exp(RandMath::log1mexp(-lambda * x) - logpdfCoef)
                        : 1.0;
}
