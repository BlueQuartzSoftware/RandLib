...
VonMisesRand<RealType>::VonMisesRand(double location, double concentration)
    : distributions::CircularDistribution<RealType>(location) {
  SetConcentration(concentration);
}

template <typename RealType> String VonMisesRand<RealType>::Name() const {
...
void VonMisesRand<RealType>::SetConcentration(double concentration) {
  if (concentration <= 0.0)
    throw std::invalid_argument(
        "von Mises distribution: concentration parameter should be positive");
  k = concentration;
...
double VonMisesRand<RealType>::f(const RealType &x) const {
  return (x < this->loc - M_PI || x > this->loc + M_PI) ? 0.0
                                                        : std::exp(logf(x));
}
...
template <typename RealType> RealType VonMisesRand<RealType>::Variate() const {
  ...
  do {
    ...
    RealType V =
        2 * UniformRand<RealType>::StandardVariate(this->localRandGenerator) -
        1;
    RealType theta = s * V / U;
    if ((std::fabs(theta) <= M_PI) &&
        ((k * theta * theta < 4.0 * (1.0 - U)) ||
         (k * std::cos(theta) >= 2 * std::log(U) + k)))
...
template <typename RealType> RealType VonMisesRand<RealType>::Median() const {
  return this->loc;
}

template <typename RealType> RealType VonMisesRand<RealType>::Mode() const {
  return this->loc;
}
