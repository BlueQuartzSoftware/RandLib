...
template <typename RealType> String UniformRand<RealType>::Name() const {
  return "Uniform(" + this->toStringWithPrecision(MinValue()) + ", " +
...
template <typename RealType> RealType UniformRand<RealType>::Variate() const {
...
RealType UniformRand<RealType>::StandardVariate(RandGenerator &randGenerator) {
  ...
  *(reinterpret_cast<unsigned long long *>(&x)) =
      a; /// Make this->a double from bits
  return x - 1.0;
#else
...
template <typename RealType>
RealType
UniformRand<RealType>::StandardVariateClosed(RandGenerator &randGenerator) {
  RealType x = randGenerator.Variate();
...
template <typename RealType>
RealType
UniformRand<RealType>::StandardVariateHalfClosed(RandGenerator &randGenerator) {
  RealType x = randGenerator.Variate();
...
template <typename RealType> long double UniformRand<RealType>::Mean() const {
...
template <typename RealType> RealType UniformRand<RealType>::Median() const {
  return 0.5 * (this->b + this->a);
}

template <typename RealType> RealType UniformRand<RealType>::Mode() const {
...
template <typename RealType>
double UniformRand<RealType>::LikelihoodFunction(
    const std::vector<RealType> &sample) const {
  bool sampleIsInsideInterval =
      this->allElementsAreNotSmallerThan(this->a, sample) &&
      this->allElementsAreNotGreaterThan(this->b, sample);
  return sampleIsInsideInterval ? std::pow(this->bma, -sample.size()) : 0.0;
}

template <typename RealType>
double UniformRand<RealType>::LogLikelihoodFunction(
    const std::vector<RealType> &sample) const {
  bool sampleIsInsideInterval =
      this->allElementsAreNotSmallerThan(this->a, sample) &&
      this->allElementsAreNotGreaterThan(this->b, sample);
...
template <typename RealType>
void UniformRand<RealType>::FitMinimum(const std::vector<RealType> &sample,
                                       bool unbiased) {
  ...
  if (unbiased) {
    ...
    if (!this->allElementsAreNotSmallerThan(minVarAdj, sample))
      throw std::runtime_error(this->fitErrorDescription(
          this->WRONG_RETURN,
          TOO_LARGE_A + this->toStringWithPrecision(minVarAdj)));
...
template <typename RealType>
void UniformRand<RealType>::FitMaximum(const std::vector<RealType> &sample,
                                       bool unbiased) {
  ...
  if (unbiased) {
    ...
    if (!this->allElementsAreNotGreaterThan(maxVarAdj, sample))
      throw std::runtime_error(this->fitErrorDescription(
          this->WRONG_RETURN,
          TOO_SMALL_B + this->toStringWithPrecision(maxVarAdj)));
...
template <typename RealType>
void UniformRand<RealType>::Fit(const std::vector<RealType> &sample,
                                bool unbiased) {
...
template <typename RealType>
ParetoRand<RealType> UniformRand<RealType>::FitMaximumBayes(
    const std::vector<RealType> &sample,
    const ParetoRand<RealType> &priorDistribution, bool MAP) {
  if (!this->allElementsAreNotSmallerThan(this->a, sample))
    throw std::invalid_argument(this->fitErrorDescription(
        this->WRONG_SAMPLE,
        this->LOWER_LIMIT_VIOLATION + this->toStringWithPrecision(this->a)));
