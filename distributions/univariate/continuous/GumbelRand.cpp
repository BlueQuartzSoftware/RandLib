...
template <typename RealType> String GumbelRand<RealType>::Name() const {
  return "Gumbel(" + this->toStringWithPrecision(GetLocation()) + ", " +
...
template <typename RealType> void GumbelRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Gumbel distribution: scale should be positive");
  beta = scale;
...
template <typename RealType> RealType GumbelRand<RealType>::Variate() const {
  return mu +
         beta * GumbelRand<RealType>::StandardVariate(this->localRandGenerator);
}
...
template <typename RealType> long double GumbelRand<RealType>::Mean() const {
...
template <typename RealType> RealType GumbelRand<RealType>::Median() const {
  static constexpr double M_LN_LN2 = nonstd::log(M_LN2);
  return mu - beta * M_LN_LN2;
}

template <typename RealType> RealType GumbelRand<RealType>::Mode() const {
  return mu;
}

template <typename RealType>
long double GumbelRand<RealType>::Skewness() const {
  static constexpr long double skew =
      12 * M_SQRT2 * M_SQRT3 * M_APERY / (M_PI_SQ * M_PI);
  return skew;
}
...
template <typename RealType> long double GumbelRand<RealType>::Entropy() const {
