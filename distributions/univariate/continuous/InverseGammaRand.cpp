...
template <typename RealType> String InverseGammaRand<RealType>::Name() const {
  return "Inverse-Gamma(" + this->toStringWithPrecision(GetShape()) + ", " +
...
void InverseGammaRand<RealType>::SetParameters(double shape, double rate) {
  if (shape <= 0.0)
    throw std::invalid_argument(
        "Inverse-Gamma distribution: shape should be positive");
  if (rate <= 0.0)
    throw std::invalid_argument(
        "Inverse-Gamma distribution: rate should be positive");
...
template <typename RealType>
void InverseGammaRand<RealType>::Sample(
...
template <typename RealType> RealType InverseGammaRand<RealType>::Mode() const {
  return beta / (alpha + 1);
}
