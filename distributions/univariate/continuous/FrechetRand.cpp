...
FrechetRand<RealType>::FrechetRand(double shape, double scale,
                                   double location) {
  SetParameters(shape, scale, location);
}

template <typename RealType> String FrechetRand<RealType>::Name() const {
...
template <typename RealType>
void FrechetRand<RealType>::SetParameters(double shape, double scale,
                                          double location) {
  if (shape <= 0.0)
    throw std::invalid_argument(
        "Frechet distribution: shape should be positive");
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Frechet distribution: scale should be positive");
  alpha = shape;
  alphaInv = 1.0 / alpha;
  s = scale;
...
template <typename RealType> RealType FrechetRand<RealType>::Variate() const {
  return m + s / std::pow(ExponentialRand<RealType>::StandardVariate(
                              this->localRandGenerator),
                          alphaInv);
}

template <typename RealType> long double FrechetRand<RealType>::Mean() const {
  if (alpha <= 1.0)
    return INFINITY;
...
template <typename RealType> RealType FrechetRand<RealType>::Median() const {
  return m + s / std::pow(M_LN2, alphaInv);
}

template <typename RealType> RealType FrechetRand<RealType>::Mode() const {
