...
template <typename RealType> String LogNormalRand<RealType>::Name() const {
  return "Log-Normal(" + this->toStringWithPrecision(GetLocation()) + ", " +
...
void LogNormalRand<RealType>::SetScale(double scale) {
  if (scale <= 0.0)
    throw std::invalid_argument(
        "Log-Normal distribution: scale should be positive");
  X.SetScale(scale);
...
template <typename RealType> RealType LogNormalRand<RealType>::Variate() const {
  return std::exp(X.Variate());
}

template <typename RealType>
RealType
LogNormalRand<RealType>::StandardVariate(RandGenerator &randGenerator) {
...
template <typename RealType> long double LogNormalRand<RealType>::Mean() const {
...
template <typename RealType> RealType LogNormalRand<RealType>::Median() const {
  return expMu;
}

template <typename RealType> RealType LogNormalRand<RealType>::Mode() const {
...
template <typename RealType>
NormalRand<RealType> LogNormalRand<RealType>::FitLocationBayes(
  ...
  NormalRand<RealType> posteriorDistribution(numerator / denominator,
                                             1.0 / denominator);
  SetLocation(MAP ? posteriorDistribution.Mode()
                  : posteriorDistribution.Mean());
  return posteriorDistribution;
}

template <typename RealType>
InverseGammaRand<RealType> LogNormalRand<RealType>::FitScaleBayes(
    const std::vector<RealType> &sample,
  ...
  SetScale(std::sqrt(MAP ? posteriorDistribution.Mode()
                         : posteriorDistribution.Mean()));
  return posteriorDistribution;
}

template <typename RealType>
NormalInverseGammaRand<RealType> LogNormalRand<RealType>::FitBayes(
    const std::vector<RealType> &sample,
  ...
  NormalInverseGammaRand<RealType> posteriorDistribution(newMu0, newLambda,
                                                         newAlpha, newBeta);
