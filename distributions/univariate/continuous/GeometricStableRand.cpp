...
void GeneralGeometricStableDistribution<RealType>::SetParameters(double exponent, double skewness, double scale, double location, double shift)
{
  if(exponent < 0.1 || exponent > 2.0)
    throw std::invalid_argument("Geometric-Stable distribution: exponent should be inside the interval [0.1, 2], but it's equal to " + std::to_string(exponent));
  if(std::fabs(skewness) > 1.0)
    throw std::invalid_argument("Geometric-Stable distribution: skewness should be inside the interval [-1, 1], but it's equal to " + std::to_string(skewness));
...
{
  if(scale <= 0.0)
    throw std::invalid_argument("Asymmetric-Laplace distribution: scale should be positive, but it's equal to " + std::to_string(scale));
  gamma = scale;
...
{
  if(asymmetry <= 0.0)
    throw std::invalid_argument("Asymmetric-Laplace distribution: asymmetry parameter should be positive, but it's equal to " + std::to_string(asymmetry));
  kappa = asymmetry;
  kappaInv = 1.0 / kappa;
  kappaSq = kappa * kappa;
