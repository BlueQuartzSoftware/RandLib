...
GeneralGeometricStableDistribution<
    RealType>::GeneralGeometricStableDistribution(double exponent,
...
template <typename RealType>
void GeneralGeometricStableDistribution<RealType>::SetParameters(
    double exponent, double skewness, double scale, double location,
    double shift) {
  if (exponent < 0.1 || exponent > 2.0)
    throw std::invalid_argument(
        "Geometric-Stable distribution: exponent should be inside the interval "
        "[0.1, 2], but it's equal to " +
        std::to_string(exponent));
  if (std::fabs(skewness) > 1.0)
    throw std::invalid_argument(
        "Geometric-Stable distribution: skewness should be inside the interval "
...
template <typename RealType>
void GeneralGeometricStableDistribution<RealType>::SetLocation(
    double location) {
  mu = location;
}
...
template <typename RealType>
void GeneralGeometricStableDistribution<RealType>::SetAsymmetry(
    double asymmetry) {
  if (asymmetry <= 0.0)
    throw std::invalid_argument(
        "Asymmetric-Laplace distribution: asymmetry parameter should be "
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::pdfLaplace(double x) const {
  return std::exp(logpdfLaplace(x));
}

template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::logpdfLaplace(double x) const {
  double y = x / gamma;
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::cdfLaplace(double x) const {
  double y = x / gamma;
  if (x < 0) {
    y *= kappaInv;
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::cdfLaplaceCompl(double x) const {
  double y = x / gamma;
  if (x < 0) {
    y *= kappaInv;
    y += 2 * logKappa - log1pKappaSq;
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::pdfByCauchy(double x) const {
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::f(const RealType &x) const {
  double x0 = x - m;
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::logf(const RealType &x) const {
  return (distributionType == LAPLACE || distributionType == ASYMMETRIC_LAPLACE)
             ? logpdfLaplace(x - m)
             : std::log(f(x));
}
...
template <typename RealType>
double GeneralGeometricStableDistribution<RealType>::variateForUnityExponent(
    double z) const {
  double W =
...
template <typename RealType>
double GeneralGeometricStableDistribution<RealType>::variateForGeneralExponent(
    double z) const {
  double W =
      ExponentialRand<RealType>::StandardVariate(this->localRandGenerator);
...
template <typename RealType>
double GeneralGeometricStableDistribution<RealType>::variateForOneHalfExponent(
    double z) const {
  double W =
      ExponentialRand<RealType>::StandardVariate(this->localRandGenerator);
...
template <typename RealType>
double
GeneralGeometricStableDistribution<RealType>::variateByCauchy(double z) const {
  double W =
      ExponentialRand<RealType>::StandardVariate(this->localRandGenerator);
  double X = mu + gamma * z;
  return X * W;
...
RealType GeneralGeometricStableDistribution<RealType>::Variate() const {
  ...
  case LAPLACE:
    return gamma *
           LaplaceRand<RealType>::StandardVariate(this->localRandGenerator);
  case ASYMMETRIC_LAPLACE:
    return gamma * AsymmetricLaplaceRand<RealType>::StandardVariate(
                       kappa, this->localRandGenerator);
  ...
  default:
    throw std::runtime_error(
        "Shifted Geometric Stable distribution: invalid distribution type");
  }
}

template <typename RealType>
void GeneralGeometricStableDistribution<RealType>::Sample(
  ...
  case ASYMMETRIC_LAPLACE: {
    for (RealType &var : outputData)
      var = gamma * AsymmetricLaplaceRand<RealType>::StandardVariate(
                        kappa, this->localRandGenerator);
  } break;
  case ONEHALF_EXPONENT:
  case LEVY: {
...
template <typename RealType>
void GeneralGeometricStableDistribution<RealType>::Reseed(
...
template <typename RealType>
std::complex<double>
GeneralGeometricStableDistribution<RealType>::CFImpl(double t) const {
  double x = 0;
  if (alpha != 2 && beta != 0) {
...
template <typename RealType>
long double
GeneralGeometricStableDistribution<RealType>::ExcessKurtosis() const {
  if (distributionType != LAPLACE && distributionType != ASYMMETRIC_LAPLACE)
    return NAN;
...
template <typename RealType>
RealType
GeneralGeometricStableDistribution<RealType>::quantileLaplace(double p) const {
...
template <typename RealType>
RealType GeneralGeometricStableDistribution<RealType>::quantileLaplace1m(
    double p) const {
  if (p > 1.0 / (1 + kappaSq)) {
    RealType pm1 = p - 1.0;
...
template <typename RealType>
GeometricStableRand<RealType>::GeometricStableRand(double exponent,
                                                   double skewness,
                                                   double scale,
                                                   double location)
    : GeneralGeometricStableDistribution<RealType>(exponent, skewness, scale,
                                                   location) {
  ChangeAsymmetry();
}
...
void GeometricStableRand<RealType>::ChangeAsymmetry() {
  if (this->distributionType != this->LAPLACE &&
      this->distributionType != this->ASYMMETRIC_LAPLACE)
    return;
...
template <typename RealType>
void GeometricStableRand<RealType>::SetParameters(double exponent,
                                                  double skewness) {
  GeneralGeometricStableDistribution<RealType>::SetParameters(exponent,
                                                              skewness);
  ChangeAsymmetry();
}
