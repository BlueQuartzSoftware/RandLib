...
template <typename RealType> String ExponentialRand<RealType>::Name() const {
...
template <typename RealType>
void ExponentialRand<RealType>::Sample(
...
template <typename RealType>
RealType
ExponentialRand<RealType>::StandardVariate(RandGenerator &randGenerator) {
  /// Ziggurat algorithm
  size_t iter = 0;
  do {
    ...
    RealType x = UniformRand<RealType>::StandardVariate(randGenerator) *
                 ziggurat[stairId].second;
    if (x < ziggurat[stairId + 1]
                .second) /// if we are under the upper stair - accept
      return x;
    ...
    if (ziggurat[stairId - 1].first +
        ...
        std::exp(-x)) /// if we are under the curve - accept
      return x;
    /// rejection - go back
