...
class RANDLIBSHARED_EXPORT NormalInverseGammaRand
    : public distributions::ContinuousBivariateDistribution<
          StudentTRand<RealType>, InverseGammaRand<RealType>, RealType> {
  double mu = 0;     ///< location μ
  double lambda = 1; ///< precision λ
  double alpha = 1;  ///< shape α
  double beta = 1;   ///< rate β
  double pdfCoef =
      0.5 * (M_LNPI - M_LN2); ///< coefficient for faster pdf calculation

public:
  NormalInverseGammaRand(double location = 0, double precision = 1,
                         double shape = 1, double rate = 1);
  String Name() const override;

  void SetParameters(double location, double precision, double shape,
                     double rate);
