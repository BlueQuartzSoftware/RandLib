...
class RANDLIBSHARED_EXPORT BivariateNormalRand
    : public distributions::ContinuousBivariateDistribution<
          NormalRand<RealType>, NormalRand<RealType>, RealType> {
  ...
  double pdfCoef =
      1 + M_LN2 + M_LNPI; ///< coefficient for faster pdf calculation
  double sqrt1mroSq = 1;  ///< √(1 - ρ)

public:
  BivariateNormalRand(double location1, double location2, double scale1,
                      double scale2, double correlation);
