...
template <typename RealType>
void NormalInverseGammaRand<RealType>::SetParameters(double location,
                                                     double precision,
                                                     double shape,
                                                     double rate) {
  if (precision <= 0.0)
    throw std::invalid_argument(
        "Precision of Normal-Inverse-Gamma distribution should be positive, "
        "but it's equal to " +
...
template <typename RealType>
double
NormalInverseGammaRand<RealType>::logf(const Pair<RealType> &point) const {
...
Pair<RealType> NormalInverseGammaRand<RealType>::Variate() const {
  ...
  var.first = mu + coef * NormalRand<RealType>::StandardVariate(
