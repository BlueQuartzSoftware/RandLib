...
TrinomialRand<IntType>::TrinomialRand(int number, double probability1,
                                      double probability2) {
  SetParameters(number, probability1, probability2);
}

template <typename IntType> String TrinomialRand<IntType>::Name() const {
...
template <typename IntType>
void TrinomialRand<IntType>::SetParameters(int number, double probability1,
                                           double probability2) {
  if (probability1 < 0.0 || probability2 < 0.0 ||
      probability1 + probability2 > 1.0)
    throw std::invalid_argument(
        "Probabilities of Trinomial distribution should be positive and their "
...
Pair<IntType> TrinomialRand<IntType>::Variate() const {
  if (this->X.GetProbability() > this->Y.GetProbability()) {
    ...
    IntType y = BinomialDistribution<IntType>::Variate(
        n - x, p2_1mp1, this->localRandGenerator);
    return IntPair(x, y);
  }
  IntType y = this->Y.Variate();
  IntType x = BinomialDistribution<IntType>::Variate(n - y, p1_1mp2,
...
long double TrinomialRand<IntType>::Correlation() const {
  long double logp1 = this->X.GetLogProbability(),
              logp2 = this->Y.GetLogProbability();
  long double log1mp1 = this->X.GetLog1mProbability(),
              log1mp2 = this->Y.GetLog1mProbability();
  return -std::exp(logp1 + logp2 - log1mp1 - log1mp2);
}

template <typename IntType> Pair<IntType> TrinomialRand<IntType>::Mode() const {
