...
template <typename T> bool UnivariateDistribution<T>::isRightBounded() const {
  SUPPORT_TYPE supp = this->SupportType();
  return (supp == LEFTSEMIFINITE_T || supp == FINITE_T);
}

template <typename T> T UnivariateDistribution<T>::Quantile(double p) const {
  if (p < 0.0 || p > 1.0)
    throw std::invalid_argument(
        "Probability p in quantile function should be in interval [0, 1]");
...
template <typename T> T UnivariateDistribution<T>::Quantile1m(double p) const {
  if (p < 0.0 || p > 1.0)
    throw std::invalid_argument(
        "Probability p in quantile function should be in interval [0, 1]");
  double minVal = this->MinValue();
...
template <typename T>
void UnivariateDistribution<T>::QuantileFunction(const std::vector<double> &p,
                                                 std::vector<T> &y) {
...
template <typename T>
void UnivariateDistribution<T>::QuantileFunction1m(const std::vector<double> &p,
                                                   std::vector<T> &y) {
  int size = std::min(p.size(), y.size());
  for (int i = 0; i != size; ++i)
...
std::complex<double> UnivariateDistribution<T>::CFImpl(double t) const {
  ...
  if (leftBound == rightBound)
    return std::complex<double>(std::cos(t * leftBound),
                                std::sin(t * leftBound));

  double re = this->ExpectedValue(
      [this, t](double x) { return std::cos(t * x); }, leftBound, rightBound);

  double im = this->ExpectedValue(
      [this, t](double x) { return std::sin(t * x); }, leftBound, rightBound);

  return std::complex<double>(re, im);
}

template <typename T>
void UnivariateDistribution<T>::CharacteristicFunction(
    const std::vector<double> &t, std::vector<std::complex<double>> &y) const {
...
template <typename T>
void UnivariateDistribution<T>::HazardFunction(const std::vector<T> &x,
...
template <typename T> T UnivariateDistribution<T>::Median() const {
  return quantileImpl(0.5);
}

template <typename T> long double UnivariateDistribution<T>::Skewness() const {
...
template <typename T> long double UnivariateDistribution<T>::Kurtosis() const {
...
template <typename T>
bool UnivariateDistribution<T>::allElementsAreNotGreaterThan(
    T value, const std::vector<T> &sample) {
...
template <typename T>
bool UnivariateDistribution<T>::allElementsAreNotSmallerThan(
    T value, const std::vector<T> &sample) {
  for (const T &var : sample) {
    if (var < value)
      return false;
...
template <typename T>
bool UnivariateDistribution<T>::allElementsAreNonNegative(
    const std::vector<T> &sample) {
  return allElementsAreNotSmallerThan(0, sample);
}

template <typename T>
bool UnivariateDistribution<T>::allElementsArePositive(
    const std::vector<T> &sample) {
  for (const T &var : sample) {
    if (var <= 0)
      return false;
...
template <typename T>
long double
UnivariateDistribution<T>::GetSampleSum(const std::vector<T> &sample) {
  return std::accumulate(sample.begin(), sample.end(), 0.0);
}

template <typename T>
long double
UnivariateDistribution<T>::GetSampleMean(const std::vector<T> &sample) {
...
template <typename T>
long double
UnivariateDistribution<T>::GetSampleLogMean(const std::vector<T> &sample) {
  long double sum = 0.0;
  for (const T &var : sample)
    sum += std::log(var);
...
template <typename T>
long double
UnivariateDistribution<T>::GetSampleVariance(const std::vector<T> &sample,
                                             double mean) {
...
template <typename T>
long double
UnivariateDistribution<T>::GetSampleLogVariance(const std::vector<T> &sample,
                                                double logMean) {
  long double sum = 0.0l;
  for (const T &var : sample) {
...
template <typename T>
LongDoublePair UnivariateDistribution<T>::GetSampleMeanAndVariance(
    const std::vector<T> &sample) {
...
template <typename T>
LongDoublePair UnivariateDistribution<T>::GetSampleLogMeanAndVariance(
    const std::vector<T> &sample) {
  /// Welford's stable method
  long double m = 0.0l, v = 0.0l;
  int n = sample.size();
...
UnivariateDistribution<T>::GetSampleStatistics(const std::vector<T> &sample) {
  ...
  for (const T &var : sample) {
    ...
    m1 += delta_k;
    m4 += term1 * delta_kSq * (k * k - 3 * k + 3) + 6 * delta_kSq * m2 -
          4 * delta_k * m3;
    ...
    if (k >= BIG_NUMBER) {
      ...
      size_t tp1 = t + 1;
      /// M4
      M4 += m4 + (DeltaSq * DeltaSq * t * (t * t - t + 1) * BIG_NUMBER) /
                     (tp1 * tp1 * tp1);
  ...
  if (res != 0) {
    ...
    /// M4
    M4 += m4 + (DeltaSq * DeltaSq * t * res * (t * t - res * t + res * res) *
                BIG_NUMBER) /
                   (tpres * tpres * tpres);
    M4 += 6 * DeltaSq * (t * t * m2 + res * res * M2) / (tpres * tpres);
    M4 += 4 * Delta * (t * m3 - res * M3) / tpres;
    /// M3
    M3 += m3 + (DeltaSq * Delta * t * res * (t - res) * BIG_NUMBER) /
                   (tpres * tpres);
...
template <class T1, class T2, typename T>
std::pair<T1, T2>
BivariateDistribution<T1, T2, T>::GetMarginalDistributions() const {
...
void ContinuousDistribution<RealType>::ProbabilityDensityFunction(
    const std::vector<RealType> &x, std::vector<double> &y) const {
  for (size_t i = 0; i != x.size(); ++i)
    y[i] = this->f(x[i]);
}

template <typename RealType>
void ContinuousDistribution<RealType>::LogProbabilityDensityFunction(
    const std::vector<RealType> &x, std::vector<double> &y) const {
  for (size_t i = 0; i != x.size(); ++i)
    y[i] = this->logf(x[i]);
}
...
template <typename RealType>
RealType
ContinuousDistribution<RealType>::quantileImpl(double p,
                                               RealType initValue) const {
  ...
  if (p < SMALL_P) {
    ...
    if (!RandMath::findRootNewtonFirstOrder<RealType>(
      ...
      throw std::runtime_error(
          "Continuous distribution: failure in numeric procedure");
    return initValue;
  }

  if (this->SupportType() == FINITE_T) {
    if (!RandMath::findRootNewtonFirstOrder<RealType>(
            [this, p](const RealType &x) { return this->F(x) - p; },
...
template <typename RealType>
RealType
ContinuousDistribution<RealType>::quantileImpl1m(double p,
                                                 RealType initValue) const {
  static constexpr double SMALL_P = 1e-5;
  if (p < SMALL_P) {
    /// for small p we use logarithmic scale
  ...
  if (this->SupportType() == FINITE_T) {
    if (!RandMath::findRootNewtonFirstOrder<RealType>(
            [this, p](const RealType &x) { return this->S(x) - p; },
            this->MinValue(), this->MaxValue(), initValue))
      throw std::runtime_error(
          "Continuous distribution: failure in numeric procedure");
    return initValue;
...
RealType ContinuousDistribution<RealType>::quantileImpl1m(double p) const {
  ...
  int index = p * SAMPLE_SIZE;
  if (index == 0.0)
    return this->quantileImpl1m(
        p, *std::max_element(sample.begin(), sample.end()));
  std::nth_element(sample.begin(), sample.begin() + index, sample.end(),
                   std::greater<>());
...
RealType ContinuousDistribution<RealType>::Mode() const {
  ...
  RealType root = 0;
  RandMath::findMin<RealType>(
      [this](const RealType &x) { return -this->logf(x); }, guess, root);
  return root;
}

template <typename RealType>
long double ContinuousDistribution<RealType>::ExpectedValue(
    const std::function<double(RealType)> &funPtr, RealType minPoint,
    RealType maxPoint) const {
  ...
  bool isLeftBoundFinite = std::isfinite(lowerBoundary),
       isRightBoundFinite = std::isfinite(upperBoundary);
...
template <typename RealType>
double ContinuousDistribution<RealType>::LikelihoodFunction(
    const std::vector<RealType> &sample) const {
  return std::exp(LogLikelihoodFunction(sample));
}

template <typename RealType>
double ContinuousDistribution<RealType>::LogLikelihoodFunction(
    const std::vector<RealType> &sample) const {
  long double res = 0.0;
...
template <typename RealType>
bool ContinuousDistribution<RealType>::KolmogorovSmirnovTest(
    const std::vector<RealType> &orderStatistic, double alpha) const {
  ...
  for (size_t i = 1; i != n; ++i) {
    ...
    if (x > orderStatistic[i])
      throw std::invalid_argument(
          "Order statistic should be sorted in ascending order");
...
void DiscreteDistribution<IntType>::ProbabilityMassFunction(
    const std::vector<IntType> &x, std::vector<double> &y) const {
  for (size_t i = 0; i != x.size(); ++i)
    y[i] = this->P(x[i]);
}

template <typename IntType>
void DiscreteDistribution<IntType>::LogProbabilityMassFunction(
    const std::vector<IntType> &x, std::vector<double> &y) const {
  for (size_t i = 0; i != x.size(); ++i)
    y[i] = this->logP(x[i]);
}
...
template <typename IntType>
IntType DiscreteDistribution<IntType>::quantileImpl(double p,
                                                    IntType initValue) const {
...
template <typename IntType>
IntType DiscreteDistribution<IntType>::quantileImpl1m(double p,
                                                      IntType initValue) const {
  IntType down = initValue, up = down + 1;
...
template <typename IntType>
long double DiscreteDistribution<IntType>::ExpectedValue(
    const std::function<double(IntType)> &funPtr, IntType minPoint,
    IntType maxPoint) const {
...
template <typename IntType>
double DiscreteDistribution<IntType>::LikelihoodFunction(
    const std::vector<IntType> &sample) const {
  long double res = 1.0;
...
template <typename IntType>
double DiscreteDistribution<IntType>::LogLikelihoodFunction(
    const std::vector<IntType> &sample) const {
  long double res = 0.0;
  for (const IntType &var : sample)
    res += this->logP(var);
...
template <typename IntType>
bool DiscreteDistribution<IntType>::PearsonChiSquaredTest(
    const std::vector<IntType> &orderStatistic, double alpha, int lowerBoundary,
    int upperBoundary, size_t numberOfEstimatedParameters) const {
  ...
  if (lowerBoundary >= upperBoundary)
    throw std::invalid_argument(
        "Lower boundary should be smaller than upper one");
  ...
  if (orderStatistic[0] < this->MinValue())
    throw std::invalid_argument(
        "Some elements in the sample are too small to belong to this "
        "distribution, they should be greater than " +
        this->toStringWithPrecision(this->MinValue()));
  if (orderStatistic[n - 1] > this->MaxValue())
    throw std::invalid_argument(
        "Some elements in the sample are too large to belong to this "
  ...
  if (lowerBoundary > this->MinValue()) {
    auto upIt = std::upper_bound(orderStatistic.begin(), orderStatistic.end(),
                                 lowerBoundary);
  ...
  if (k <= numberOfEstimatedParameters + 1) {
    throw std::invalid_argument(
        "Sample is too small, number of groups (" +
...
template <typename IntType>
bool DiscreteDistribution<IntType>::PearsonChiSquaredTest(
    const std::vector<IntType> &orderStatistic, double alpha,
    size_t numberOfEstimatedParameters) const {
  return PearsonChiSquaredTest(orderStatistic, alpha, this->MinValue(),
                               this->MaxValue(), numberOfEstimatedParameters);
}

//-------------------------------------------------------------------------------------------
// SINGULAR
//-------------------------------------------------------------------------------------------
double SingularDistribution::Hazard(const double &) const { return NAN; }

long double
SingularDistribution::ExpectedValue(const std::function<double(double)> &,
                                    double, double) const {
  return NAN;
}

double SingularDistribution::Mode() const { return NAN; }

double
SingularDistribution::LikelihoodFunction(const std::vector<double> &) const {
  return NAN;
}

double
SingularDistribution::LogLikelihoodFunction(const std::vector<double> &) const {
  return NAN;
}
} // namespace distributions