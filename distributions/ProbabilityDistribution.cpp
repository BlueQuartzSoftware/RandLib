...
template <typename T> ProbabilityDistribution<T>::ProbabilityDistribution() {}

template <typename T>
String ProbabilityDistribution<T>::toStringWithPrecision(const double a_value,
                                                         const int n) const {
...
template <typename T>
void ProbabilityDistribution<T>::CumulativeDistributionFunction(
    const std::vector<T> &x, std::vector<double> &y) const {
  size_t size = x.size();
  if (size > y.size())
    return;
...
template <typename T> double ProbabilityDistribution<T>::S(const T &x) const {
  return 1.0 - this->F(x);
}

template <typename T>
void ProbabilityDistribution<T>::SurvivalFunction(
    const std::vector<T> &x, std::vector<double> &y) const {
  size_t size = x.size();
  if (size > y.size())
    return;
...
template <typename T>
String ProbabilityDistribution<T>::fitErrorDescription(
    ProbabilityDistribution::FIT_ERROR_TYPE fet, const String &explanation) {
